...
| Fixture | Platform | Speaker Detection | Reviewed |
|---------|----------|-------------------|----------|
...
| google_aistudio_image.html | AI Studio | `data-turn-role` | **TODO** |
| google_aistudio_ux_discussion.html | AI Studio | `data-turn-role` | **TODO** |
| google_gemini_debug.html | Gemini | `<user-query>` / `<model-response>` | **TODO** |
| google_gemini_deep_research.html | Gemini | `<user-query>` / `<model-response>` | **TODO** |
| openai_biblatex.html | OpenAI | `items-end` / `agent-turn` | **TODO** |
| openai_dh_dr.html | OpenAI | `items-end` / `agent-turn` | **TODO** |
| openai_dprk_denmark.html | OpenAI | `items-end` / `agent-turn` | **TODO** |
| openai_software_long_dr.html | OpenAI | `items-end` / `agent-turn` | **TODO** |
| scienceos_loc.html | ScienceOS | N/A (research report) | **TODO** |
| scienceos_philsci.html | ScienceOS | N/A (research report) | **TODO** |
| austlii.html | AustLII | N/A (legal document) | ✓ |
| chinese_wikipedia.html | N/A | N/A (CJK test) | **TODO** |
| translation_japanese_sample.html | N/A | N/A (CJK test) | **TODO** |
| translation_korean_sample.html | N/A | N/A (CJK test) | **TODO** |
| translation_spanish_sample.html | N/A | N/A (translation test) | **TODO** |

**Reviewed column:** Manual visual review required before PR merge. Automated tests verify compilation, human review confirms output quality.
...
**Fixed: luatexja "Missing font identifier" error**

Root cause: The `[match]` option in `luatexja-fontspec` caused font switching conflicts when Western font families changed (e.g., `\texttt{}` in section headings triggered CJK Sans font lookup, which failed).

**Changes:**
- ✓ Removed `[match]` option from `\usepackage{luatexja-fontspec}` in `unicode_latex.py`
- ✓ Added `_strip_texorpdfstring()` in `latex.py` using pylatexenc for defense-in-depth
- ✓ Made CJK font specifications explicit with all font faces (Upright, Bold, Italic, BoldItalic)
- ✓ All 40 chatbot fixture tests now pass (including CJK: chinese, japanese, korean)

**Stubbed:**
- `tests/e2e/test_pdf_export.py` - Documented workflow for reimplementation, blocked on #106

**Pending visual review:**
- All fixtures in matrix marked **TODO** need human review of PDF output
