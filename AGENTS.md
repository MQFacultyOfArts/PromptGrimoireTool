...
- **8-Lane Model**: Tests are split into 8 lanes (js, bats, unit, integration, playwright, nicegui, smoke, blns+extra). `test all` runs BATS + JS + unit. `e2e all` runs all 8 lanes. `e2e slow` is a superset of `e2e all` that additionally runs Playwright with latexmk enabled, compiled-PDF validation, and `noci`-marked tests included. The default Playwright lane (`e2e run`) excludes tests marked `noci` (heavy browser scenarios reserved for nightly CI). A nightly GitHub Actions workflow runs `e2e slow`. E2E tests (Playwright) contaminate xdist workers and must never run in unit/integration lanes. JS tests (`tests/js/`) use vitest + happy-dom. BATS tests cover shell scripts in `deploy/tests/`.
...
**Tag/document deletion guards** (`db/tags.py`, `db/workspace_documents.py`): Deletion functions enforce precondition guards before allowing destructive operations. `delete_tag_group` raises `HasChildTagsError` if the group contains tags. `delete_tag` reads persisted CRDT state to count highlights referencing the tag, raising `HasHighlightsError` if any exist. `delete_document` reads persisted CRDT state to count highlights on the document, raising `HasAnnotationsError` if any exist. UI delete buttons are disabled with explanatory tooltips when guards would block (`_apply_deletion_guard` in `tag_management_rows.py`).
