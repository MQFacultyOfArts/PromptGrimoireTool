...
|---------|--------|-------------|---------------|
| Authenticate Users | 1 | Transforms auth tokens and identity metadata into session state, user records, and derived roles (AAF eduperson_affiliation → instructor/student). Handles magic links, passkeys, OAuth, SSO. Enforces user bans: checks `is_banned` at session validation, maintains in-memory client↔user registry for real-time disconnect with cross-instance ban broadcast via Redis Pub/Sub (`bans` channel), exposes internal `/api/admin/kick` endpoint for CLI-triggered kicks. | Leaf process |
...
|-------|-------------|---------|------------|
| D1: Session State | Auth sessions (tokens, WebSocket bindings), user records (including `is_banned`, `banned_at`), derived roles, course enrollments, in-memory client↔user registry, workspace ownership claims. Session data in Redis (`app.storage.user` via `NICEGUI_REDIS_URL`) for cross-instance sharing; workspace ownership via Redis `SETNX` with TTL; persistent user/enrollment records in PostgreSQL. | All processes (identity + permissions) | P1 Authenticate Users |
