...
- file-upload-109.AC1.3: Corrupt/empty DOCX returns ConversionError
- file-upload-109.AC2.1: PDF produces HTML with paragraph structure
- file-upload-109.AC2.3: Corrupt/empty PDF returns ConversionError
"""
...
class TestConvertDocxToHtml:
    ...
    def test_corrupt_docx_raises_conversion_error(self) -> None:
        """AC1.3: Invalid bytes raise ConversionError."""
        with pytest.raises(ConversionError, match=r"(?i)docx"):
            convert_docx_to_html(b"not a docx file")

    def test_empty_bytes_raises_conversion_error(self) -> None:
        """AC1.3: Empty bytes raise ConversionError."""
        with pytest.raises(ConversionError, match=r"(?i)docx"):
...
class TestConvertPdfToHtml:
    ...
    @pytest.mark.asyncio
    async def test_corrupt_pdf_raises_conversion_error(self) -> None:
        """AC2.3: Invalid bytes raise ConversionError."""
        with pytest.raises(ConversionError, match=r"(?i)pdf"):
            await convert_pdf_to_html(b"not a pdf file")

    @pytest.mark.asyncio
    async def test_empty_bytes_raises_conversion_error(self) -> None:
        """AC2.3: Empty bytes raise ConversionError."""
        with pytest.raises(ConversionError, match=r"(?i)pdf"):
            await convert_pdf_to_html(b"")

    @pytest.mark.asyncio
    async def test_pandoc_failure_raises_conversion_error(self) -> None:
        ...
        with (
            ...
            pytest.raises(ConversionError, match=r"pandoc failed"),
        ):
