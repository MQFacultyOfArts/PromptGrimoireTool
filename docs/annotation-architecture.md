...
The annotation page (`pages/annotation/`) is a 13-module package split from a monolith.
...
- `test_annotation_package_structure.py` -- Prevents regression: package directory exists, monolith `.py` file absent, all 13 modules present, no satellite files at `pages/` level, no imports from old paths
