...
Note: The autodiscovery and entry point implementation is already complete in Phase 1's `__init__.py`. Phase 4 adds comprehensive tests for the registry behaviour.

---

<!-- START_SUBCOMPONENT_F (tasks 1-3) -->
<!-- START_TASK_1 -->
### Task 1: Write failing tests for registry autodiscovery

**Files:**
- Create: `tests/unit/export/platforms/test_registry.py`
...
class TestImportFailureHandling:
    ...
    def test_import_failure_logs_and_continues(
        self, caplog: pytest.LogCaptureFixture
    ) -> None:
...
class TestPlatformHandlerProtocol:
    ...
    def test_all_handlers_implement_protocol(self) -> None:
        ...
        for name, handler in _handlers.items():
            assert isinstance(handler, PlatformHandler), f"{name} doesn't implement protocol"
    ...
    def test_handler_names_match_registry_keys(self) -> None:
        ...
        for key, handler in _handlers.items():
            assert handler.name == key, f"Handler {key} has mismatched name {handler.name}"
```

**Step 2: Run tests to verify they fail**

Run: `uv run pytest tests/unit/export/platforms/test_registry.py -v`
Expected: FAIL - handlers not yet implemented (Phases 2-3 not yet executed)
<!-- END_TASK_1 -->

<!-- START_TASK_2 -->
### Task 2: Verify registry tests pass after Phase 1-3 implementation

This task executes after Phases 1-3 are implemented. The tests from Task 1 should pass once all handlers are created.
...
**Step 2: Run all platform tests together**
...
<!-- START_TASK_3 -->
### Task 3: Verify and commit registry tests

**Step 1: Run type checker**

Run: `uvx ty check tests/unit/export/platforms/test_registry.py`
Expected: No type errors

**Step 2: Commit**

```bash
git add tests/unit/export/platforms/test_registry.py
git commit -m "test(export): add registry autodiscovery and dispatch tests

Tests platform handler registry:
- Autodiscovery finds all 5 handlers
- get_handler dispatches correctly
- preprocess_for_export entry point works
- platform_hint override with fallback
