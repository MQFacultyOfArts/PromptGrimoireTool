...
class TestCanDeleteDocument:
    """Verify document delete eligibility logic (AC4.3)."""

    def test_owner_can_delete_user_uploaded_document(
        self, make_workspace_document
    ) -> None:
        """Owner can delete a document with NULL source_document_id."""
        doc = make_workspace_document(
            workspace_id=_OWNER_WORKSPACE_ID,
            title="My Upload",
            source_document_id=None,
        )
        assert can_delete_document(doc, is_owner=True) is True
    ...
    def test_owner_cannot_delete_template_cloned_document(
        ...
        assert can_delete_document(doc, is_owner=True) is False
    ...
    def test_non_owner_cannot_delete_any_document(
        ...
        assert can_delete_document(doc, is_owner=False) is False
